...
from update_crypto_screeners import update_crypto_screener


# 1- filter screeners per crypto with ranking
# 2- extract a limit number of the screeners for the same crypto
#  

def filter_cryptos(func1,func2,re_pattern,screener_data, max_exchanges,cmc_rank_symbol):

    matches_nb = 0
   # screener_list_reduced = []
    print('length screeners_data',len(screener_data))

    matched_stablecoin_list = []
    matched_stablecoin_pairs = {}
    # try to match with stablecoins pair
    for screener in screener_data:
        # extract the pair from the fata
        pair = screener[1]
       # print('pair',pair)
        ...
        if matches_nb < max_exchanges/2:
            ...
            if func1(re_pattern,pair):
                ...
                new_screener = screener + (cmc_rank_symbol,)
...
def match_screeners_with_rank(re_pattern,screeners,max_exchanges,cmc_rank):
    # filerting crypto per symbol and ranking crypto 
    screeeners_filtered_by_rank = list(filter_cryptos(is_crypto_matched_with_stablecoins,is_crypto_matched,re_pattern,screeners,max_exchanges,cmc_rank))
...
#def filter_screeners_by_ranking(crypto_symbol, screeners,max_exchanges,cmc_rank):
def filter_screeners_by_ranking(crypto, screeners,max_exchanges):    
    ...
    reduced_screeners_list = match_screeners_with_rank(re_pattern,screeners,max_exchanges,cmc_rank)
...
def reduce_screeners_by_ranking(rank):
   ...
   # cryptos_by_rank_test = (cryptos_by_rank[0],cryptos_by_rank[1])

    max_exchanges = 5

    # reduce the list of screener crypto to a list of a set of 'max_exchanges' screener per symbol
    reduced_cryptos_with_ranking = [filter_screeners_by_ranking(crypto,crypto_screeners,max_exchanges) for crypto in cryptos_by_rank]
    #reduced_cryptos_with_ranking = list(filter_screeners_by_ranking((x['symbol'],crypto_screeners,max_exchanges,x['cmc_rank']) for x in cryptos_by_rank_test]
    #reduced_cryptos_with_ranking = [filter_screeners_by_ranking(x['symbol'],crypto_screeners,max_exchanges,x['cmc_rank']) for x in cryptos_by_rank_test]
    
    # removing empty list item from the list
    #reduced_cryptos_with_ranking_empyty_removed = list(filter(lambda x: len(x) > 0 ,reduced_cryptos_with_ranking))
    print('reduced_cryptos_with_ranking', reduced_cryptos_with_ranking)
